...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    [_WIN] = { ENCODER_CCW_CW(KC_VOLD, KC_VOLU), ENCODER_CCW_CW(BL_DOWN, BL_UP), ENCODER_CCW_CW(C(MS_WHLD), C(MS_WHLU))},
    [_MAC] = { ENCODER_CCW_CW(KC_TRNS, KC_TRNS), ENCODER_CCW_CW(KC_TRNS, KC_TRNS), ENCODER_CCW_CW(G(KC_MINS), G(KC_EQL))},
    [_WIN_INDEX] = { ENCODER_CCW_CW(KC_TRNS, KC_TRNS), ENCODER_CCW_CW(KC_TRNS, KC_TRNS), ENCODER_CCW_CW(C(MS_WHLD), C(MS_WHLU))},
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
       ...
       case DF(_MAC):
            if (record->event.pressed){
               set_single_persistent_default_layer(_MAC); // Save default layer 0 to eeprom
           }
         return false;
